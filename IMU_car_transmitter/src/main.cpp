...
// uint8_t receiverMacAddress[] = {0xEC,0x62,0x60,0x83,0xC3,0xF8};  //AC:67:B2:36:7F:28
// uint8_t receiverMacAddress[] = {0xFC,0xB4,0x67,0x51,0x24,0x40};
uint8_t receiverMacAddress[] = {0xE8,0xDB,0x84,0x00,0xDB,0xC0};
// EC:62:60:83:C3:F8

// FC:B4:67:51:24:40 ken

// E8:DB:84:00:DB:C0 hoi lam
struct PacketData 
{
  byte xAxisValue;
  byte yAxisValue;
...
void setup() {
  ...
  Wire.requestFrom(MAG_ADDR, 3 , 1);  //GET SENSITIVITY ADJUSMENT VALUES STARTS AT ASAX
...
void loop() {
  ...
  Wire.endTransmission(false);
  Wire.requestFrom(IMU_ADDR, 14, 1); //request a total of 14 bytes
...
// // === end of Printing data to serial port ===//
  ...
  // read a packet from FIFO. Get the Latest packet
    int xAxisValue = mag_X;
    int yAxisValue = mag_Y;
    int zAxisValue = mag_Z; 
